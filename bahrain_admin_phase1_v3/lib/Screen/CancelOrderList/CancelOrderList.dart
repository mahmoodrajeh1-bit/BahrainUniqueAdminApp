...
class _CancelOrderListState extends State<CancelOrderList> {
  ...
  Future<void> _showOrders() async {
    // var key = 'cancel-orders-list';
    // await _getLocalOrderData(key);

    var res = await CallApi().getData(
        '/app/indexOrder?status=Cancel'); //order=$_currentStatusSelected,$_sendType');
...
//////////////// get more orders start ////////////////
  _loadMoreCancelledOrders() async {
    setState(() {
      _isLoadingMore = true;
    });

    var res = await CallApi()
        .getData('/app/indexOrder?status=Cancel&lastId=$_lastId');
    print('res - $res');
  ...
  Future<void> _showSearchOrders(String fr, String to) async {
    ...
    // var key = 'Cancel-date-orders-list';
    //await _getLocalOrderData(key);

    var res = await CallApi()
        .getData('/app/indexOrder?status=Cancel&date1=$fr&date2=$to');
    body = json.decode(res.body);
  ...
  Future<void> _showMoreSearchOrders(String fr, String to) async {
    ...
    var res = await CallApi().getData(
        '/app/indexOrder?status=Cancel&date1=$fr&date2=$to&lastId=$_lastSearchId');
    body = json.decode(res.body);
