...
class CouponsOrderList extends StatefulWidget {
  final id;
  CouponsOrderList(this.id);
  @override
  _CouponsOrderListState createState() => _CouponsOrderListState();
}

class _CouponsOrderListState extends State<CouponsOrderList> {
  ...
  Future<void> _showOrders() async {
    ...
    // var key = 'coupons-orders-list';
    //await _getLocalOrderData(key);

    var res = await CallApi().getData(
        '/app/getCouponDetailsById?voucherId=${widget.id}'); //order=$_currentStatusSelected,$_sendType');
  ...
  void _deleteOrders(var d) async {
    ...
    if (body['success'] == true) {
      ...
      //  Navigator.of(context).pop();
      Navigator.push(context, FadeRoute(page: CouponsOrderList("")));
    }
