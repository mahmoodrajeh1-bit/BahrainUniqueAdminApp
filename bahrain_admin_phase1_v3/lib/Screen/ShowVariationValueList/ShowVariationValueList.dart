...
class ShowVariationValueList extends StatefulWidget {
  @override
  _ShowVariationValueListState createState() => _ShowVariationValueListState();
}

class _ShowVariationValueListState extends State<ShowVariationValueList> {
...
bool  _isLoading = true;
 ...
 Future<void> _allData() async {
  ...
  void _bestProductsState() {
    var products = ShowVariationValueModel.fromJson(body);
    if (!mounted) return;
    setState(() {
      productsData = products.productVariationValue;
      _isLoading = false;
    });
  ...
  Future<void> _showProducts() async {
    var key = 'products-variations-value-list';
    await _getLocalBestProductsData(key);

    var res = await CallApi().getData('/app/indexproductVariationValue');
  ...
  Navigator.push( context, FadeRoute(page: ProductTypeView()));
        ...
        body:_isLoading?Center(
          ...
          child: GestureDetector(
             onTap: (){
  Navigator.push( context, FadeRoute(page: AddProductVariationValue()));
