...
//BuildContext context;
 ...
 // void _pickAsset(PickType type, {List<AssetPathEntity> pathList}) async {
  ...
  //   List<AssetEntity> photoList = await PhotoPicker.pickAsset(
  ...
  //         var file = await e.file;
  //           i++;
  //         _uploadImg(file.absolute.path, photoList.length, i);
  ...
  _pickVideo() async {
   ...
   final pickedVideo = await imgPick.ImagePicker().pickVideo(source: imgPick.ImageSource.gallery);
    File? video = pickedVideo == null ? null : File(pickedVideo.path); // ImagePicker.pickVideo(source: ImageSource.gallery);
    if (video == null) {
      print("null");
    } else {
        print("video");
...
//     int first = 1;
  ...
  void _uploadImg(filePath, int len, int index) async {
    ...
    try {
      ...
      print("File base name img: $fileName");
